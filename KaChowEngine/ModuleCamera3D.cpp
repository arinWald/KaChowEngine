...
bool ModuleCamera3D::Init()
{
	...
	sceneCam->frustrumCamera.pos = float3(0, 2, -10);
...
update_status ModuleCamera3D::Update(float dt)
{
	...
	if (App->input->GetKey(SDL_SCANCODE_Q) == KEY_REPEAT) sceneCam->frustrumCamera.pos.y += speed;
	if (App->input->GetKey(SDL_SCANCODE_E) == KEY_REPEAT) sceneCam->frustrumCamera.pos.y -= speed;

	if (App->input->GetKey(SDL_SCANCODE_S) == KEY_REPEAT) sceneCam->frustrumCamera.pos += sceneCam->frustrumCamera.front * speed;
	if (App->input->GetKey(SDL_SCANCODE_W) == KEY_REPEAT) sceneCam->frustrumCamera.pos -= sceneCam->frustrumCamera.front * speed;


	if (App->input->GetKey(SDL_SCANCODE_A) == KEY_REPEAT) sceneCam->frustrumCamera.pos -= sceneCam->frustrumCamera.WorldRight() * speed;
	if (App->input->GetKey(SDL_SCANCODE_D) == KEY_REPEAT) sceneCam->frustrumCamera.pos += sceneCam->frustrumCamera.WorldRight() * speed;
